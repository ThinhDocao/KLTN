...
PlateFinder::~PlateFinder(void)  //Hàm hủy
{
...
void PlateFinder::ImageRestoration(char* Anh)
{
	...
	Mat img_color = imread(Anh, IMREAD_GRAYSCALE);
	Mat face_roi;
	Mat BlackWhite;
	Mat Blur;
	//Cắt biển số 
	...
	/*Ptr<SVM> svm = Algorithm::load<SVM>("svm.txt");
	imshow("123123", svm->getVarCount());
